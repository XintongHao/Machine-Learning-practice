...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    dscore = P
    dscore[range(X.shape[0]), y] -= 1 # [PLEASE IMPLEMENT] partial derivative of loss wrt. the logits (dL/dz)
    ...
    if self.activation is 'relu':
      dhidden_term = dhidden * (hidden>0)
        # [PLEASE IMPLEMENT]
        # raise NotImplementedError('ReLU backward not implemented')
    elif self.activation is 'sigmoid':
      dhidden_term = dhidden * hidden * (1 - hidden)
      # 1 / (1 + np.exp(-hidden))
        # [PLEASE IMPLEMENT]
    ...
    # first layer
    dW1 = np.dot(X.T, dhidden_term) / N  # [PLEASE IMPLEMENT]
    db1 = np.mean(dhidden_term, axis=0)   # [PLEASE IMPLEMENT]
    ###########################################################################
    #                            END OF YOUR CODE
    ###########################################################################
